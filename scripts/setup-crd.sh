...
# renovate:github-url
kustomize build https://github.com/kubernetes-sigs/aws-load-balancer-controller//config/default?ref=v2.14.0 > crd.yaml
python3 openapi2jsonschema.py crd.yaml && rm crd.yaml
...
helm template --include-crds oci://public.ecr.aws/karpenter/karpenter --set settings.clusterName=test --version "1.7.2" > crd.yaml
python3 openapi2jsonschema.py crd.yaml && rm crd.yaml
...
helm template --include-crds --repo https://victoriametrics.github.io/helm-charts/ --set settings.clusterName=test victoria-metrics-k8s-stack --version "0.61.5" > crd.yaml
python3 openapi2jsonschema.py crd.yaml && rm crd.yaml
